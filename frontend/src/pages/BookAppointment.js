...
export default function BookAppointment() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await bookAppointment({ ...form, date: toDateStr(selectedDate), timeSlot: selectedSlot });
      setDone(true);
  ...
  if (done) return (
    <div className="min-h-[60vh] flex items-center justify-center bg-gray-50">
      <div className="card max-w-md w-full mx-4 text-center">
        ...
        <p className="text-gray-500 mb-4">We've sent a confirmation to <strong>{form.email}</strong>. Our team will confirm your appointment shortly.</p>
  ...
  return (
    <>
      <SEO title="Book Appointment" description="Schedule a free 30-minute consultation with our CA experts. Pick a date and time that works for you." canonical="/book-appointment" />
      ...
      <section className="bg-gradient-to-br from-blue-900 to-blue-700 text-white py-14">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-blue-100">Schedule a free 30-minute consultation with our CA experts.</p>
          {/* Steps */}
            ...
            <div className="card">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[['name','Name *','Your full name','text'],['phone','Phone *','+91 98765 43210','tel']].map(([n,l,p,t]) => (
                    <div key={n}>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{l}</label>
                      <input name={n} type={t} value={form[n]} onChange={handleFormChange} placeholder={p}
                ...
                <div>
                  ...
                  <textarea name="message" value={form.message} onChange={handleFormChange} rows={3} placeholder="Brief description..."
                    className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
                </div>
              </div>
              <div className="flex justify-between mt-6">
                <button onClick={() => setStep(1)} className="btn-outline text-sm">← Back</button>
                <button onClick={() => setStep(3)} className="btn-primary text-sm">Review Booking <ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
          )}
              ...
              <div className="bg-gray-50 rounded-xl p-5 space-y-3 mb-6 text-sm">
                {[
                  ...
                  <div key={k} className="flex gap-3">
                    <span className="font-semibold text-gray-600 w-20 flex-shrink-0">{k}:</span>
                    <span className="text-gray-800">{v}</span>
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-400 mb-5">By confirming, you agree to our terms. We'll send a confirmation email and our team will follow up within 1 hour.</p>
